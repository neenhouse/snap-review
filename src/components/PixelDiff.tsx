...
export default function PixelDiff({
      ...
      if (cDiff && cBefore && cAfter) {
        ...
        for (let i = 0; i < dataBefore.data.length; i += 4) {
          ...
          if (diff > threshold) {
            ...
            diffImage.data[i + 1] = 60;
            diffImage.data[i + 2] = 172;
  ...
  useEffect(() => {
    ...
    annotations.forEach((ann, i) => {
      ...
      ctx.fillStyle = 'rgba(255, 60, 172, 0.9)';
      ctx.fill();
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 2;
      ctx.stroke();
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // The canvas is scaled to fit the container
    const displayCanvas =
      mode === 'diff-only'
    ...
    // For side-by-side, use the after canvas
  ...
  return (
    <div className="pixel-diff" data-testid="pixel-diff">
      ...
      <div className="diff-controls">
        ...
        {diffStats && (
          <div className="diff-stats">
            <span className="stat">
              <strong>{diffStats.changed.toLocaleString()}</strong> px changed
            </span>
            <span className="stat">
              <strong>{diffStats.pct}%</strong> different
            </span>
          </div>
        )}
      </div>
      ...
      <div className="diff-viewport">
        {mode === 'side-by-side' && (
          <div className="diff-side-by-side" onClick={handleCanvasClick}>
            ...
            <div className="diff-panel">
              <span className="diff-panel-label">After</span>
      ...
      <p className="diff-hint">Click on the image to add annotation pins.</p>
    </div>
  );
}
